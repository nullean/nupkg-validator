...
let private generatePackages (arguments: ParseResults<Arguments>) =
    ...
    let staging = Paths.RootRelative <| Path.Combine(Paths.Output.FullName, "..", "validator-staging")
...
let private validatePackages (arguments: ParseResults<Arguments>) =
    ...
    let validationArgs =
        [ "validate"
          nugetPackage
          "-v"
          currentVersionInformational.Value
          "-a"
          Paths.ToolName
          "-k"
          "96c599bbe3e70f5d" ]
    printfn "dotnet %A" (dotnetRun @ [ "--" ] @ validationArgs)
